...
def test_cli_list_packages_help():
...
def test_build_command_success(mock_package_builder):
    ...
    mock_builder.build_all_packages.return_value = ['/path/to/package1', '/path/to/package2']
...
@patch('unity_wrapper.cli.PackageBuilder')
def test_build_command_specific_package(mock_package_builder):
...
def test_list_packages_command(mock_config_manager):
    ...
    mock_manager.get_nuget_package_config.return_value = {
        'nuget_id': 'TestPackage', 'version': '1.0.0', 'framework': 'netstandard2.0'
    }
...
@patch('unity_wrapper.cli.ConfigManager')
def test_list_packages_command_empty(mock_config_manager):
...
@patch('unity_wrapper.cli.PackageBuilder')
def test_check_command(mock_package_builder):
    """Test check command."""
    mock_builder = MagicMock()
    mock_package_builder.return_value.__enter__.return_value = mock_builder
...
@patch('unity_wrapper.cli.PackageBuilder')
def test_check_command_no_updates(mock_package_builder):
